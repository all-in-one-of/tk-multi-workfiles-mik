...
class MultiWorkFiles(tank.platform.Application):
    ...
    def init_app(self):
        ...
        if self._can_change_version:
            ...
            self.engine.register_command("Version up Current Scene...", self.version_up_shortcut)
    ...
    def version_up_shortcut(self):
        """
        If save as is available, show the change version dialog.
        """
        if self._can_change_version:
            tk_multi_workfiles = self.import_module("tk_multi_workfiles")
            return tk_multi_workfiles.Versioning.change_version(self)
        else:
            return False

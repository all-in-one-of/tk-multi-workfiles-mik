...
class FileFinder(object):
    ...
    def find_files(self, work_template, publish_template, context, filter_file_key=None):
        ...
        # self.__app.log_debug(work_versions)
        # self.__app.log_debug(published_files)
        ...
        self.__app.log_debug(str(len(published_files)))
        for published_file in published_files:
            ...
            try:
                ...
                self.__app.log_exception("%s"%str(e))
            ...
            self.__app.log_debug("wp: %s"%work_path)
            self.__app.log_debug("file_details: %s"%str(file_details))
            self.__app.log_debug("file_key: %s"%str(file_key))
            ...
            self.__app.log_debug("file Item: %s"%str(existing_file_item))

            try:
                if existing_file_item is not None and existing_file_item.is_local:
    ...
    def __find_work_files(self, context, work_template):
        ...
        # execute the hook - this will return a list of path for wip files found

        skip_fields = self.__app.get_setting("wip_skip_fields", [])
        try:
            work_file_paths =  self.__app.sgtk.paths_from_template(work_template, work_fields,skip_fields ,skip_missing_optional_keys=True)
        except Exception, e:
            self.__app.log_exception("%s"%str(e))
            raise TankError(str(e))

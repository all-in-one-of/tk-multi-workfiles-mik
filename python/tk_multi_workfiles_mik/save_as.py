...
class SaveAs(object):
                    ...
                    try:
                        ...
                        for counter in range(0, counter_limit):
                            ...
                            try:
                                ...
                                self._app.log_exception("%s"%str(e))
    ...
    def generate_new_work_file_path(self, current_path, current_is_publish, new_name, reset_version):
        ...
        users = list(set([f.modified_by['login'] for f in self._cached_files]))
        file_keys = []
        ...
        for user in users:
            ...
            key_fields["cs_user_name"] = user
            file_key = self._work_template.apply_fields(key_fields)
            file_keys.append(file_key)


        # find the max work file and publish versions:
        work_versions = [f.version for f in self._cached_files if f.is_local and f.key in file_keys]
        max_work_version = max(work_versions) if work_versions else 0
        publish_versions = [f.version for f in self._cached_files if f.is_published and f.key  in file_keys]
            ...
            if current_is_publish and ((not has_name_field) or new_name == current_name):
                ...
                if max_version:
                    ...
                    if max_work_version > max_publish_version:
                        if has_name_field:
                            msg = ("This variant already exists in your work folder.  Using"
                        ...
                        if has_name_field:
                            msg = ("This variant has already been published.  Using"
                                " next available version number.")
                        else:
        ...
        # create the new path
        # self._app.log_info(fields)
        new_work_path = self._work_template.apply_fields(fields)
        self._app.log_info(new_work_path)
